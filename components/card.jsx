...
export default function CardReadMore({ title, items = [] }) {
    return (
        <section className="relative border-2 border-cyan-100 px-10 ml-2 mr-4 py-6 transition-transform transform hover:-translate-y-6 duration-300 ease-in-out group hover:bg-black">
            ...
            {items.map((item, index) => (
                <div key={index} className="font-normal text-base leading-6 text-[#d2d2d3] flex items-baseline pl-7.5 py-4">
                    ...
                    <p className="leading-6 tracking-wide">{item}</p>
                </div>
            ))}
